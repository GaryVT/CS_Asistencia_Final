...
class UsuarioModel:
    ...
    def crear_usuario_v2(self,dni, contrasena, nombre, apellido, tipousuario):    
        data = {
            'dni' : dni,
            'contrasena' : contrasena,
            'nombre' : nombre,
        ...
        query = """insert into usuario (dni, contrasena, nombre, apellido, tipousuario) 
            values (%(dni)s, %(contrasena)s, %(nombre)s, %(apellido)s, %(tipousuario)s)"""    
        cursor = self.mysql_pool.execute(query, data, commit=True)   

        return data

    def actualizar_usuario_v2(self, id_usuario, dni, contrasena, nombre, apellido, tipousuario):     
        data = {
    ...
    def actualizar_usuario(self, id_usuario, nombre, apellido, tipousuario, contrasena, dni ,fotoSistema):    
        data = {
            'id_usuario' : id_usuario,
            'dni' : dni,
            'contrasena' : contrasena,
    ...
    def mostrar_usuarios(self):  
        ...
        content = {}
        for result in rv:
            content = {'id_usuario': result[0], 'dni': result[1], 'contrasena': result[2], 'nombre': result[3], 'apellido': result[4], 'tipousuario': result[5], 'fotoSistema': result[6]}
            data.append(content)
            content = {}
        return data
